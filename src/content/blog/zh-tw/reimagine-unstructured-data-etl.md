...
image:
  ...
  alt: '試著想像一下，一種能夠理解任何物理訊號，並且有智慧地回應任何人類語言指令的電腦系統。'
tags: ['新創公司', '產品']
---
...
workflows。但這一趨勢是否會繼續改變包括現代資料堆疊（Modern Data
...
我的學術背景是電腦視覺（CV，Computer Vision）和機器學習（Machine Learning，ML），一路上見證了這些領域從傳統的 CV 演算法，如方向梯度直方圖（Histogram
of Oriented Gradients，HoG）和尺度不變特徵變換（Scale-Invariant Feature Transform，SIFT），到經典的 ML 方法如支持向量機（Support Vector
...
最讓我印象深刻的不是這些基礎模型有多強大，而是它們在軟體工具中的顛覆性。我們看到 OpenAI Research 可以將外部網路搜索整合進 agentic
workflows 中，對某個主題進行深入研究，OpenAI-o1 和 DeepSeek-R1 在推理過程中進行自我反思以提高推理和答案的準確性，OpenAI Operator 和 Anthropic Computer
Use 通過控制使用者的鍵盤和滑鼠執行機器人流程自動化（RPA，Robotic Process Automation）任務。

自 2012 年以來，第一次 DL 熱潮圍繞[**卷積神經網路（ConvNets）**](https://en.wikipedia.org/wiki/Convolutional_neural_network)及其在 CV 中的 ImageNet 資料集的突破。（當時，這其實已經是第三次 AI 熱潮，緊隨 Symbolic
...
從那時起，我們見證了基礎模型的全球競爭。最初，競爭集中在美國和歐盟，Anthropic、Cohere、Mistral 和 Google 是主要參與者。現在，中國的 AI 公司如 DeepSeek 和 MiniMax 也加入了競爭，提供了 30 倍便宜的基礎模型 API 價格。產業趨勢很明顯：_基礎模型正在迅速商品化，正在快速轉變為軟體平台和應用程式可以從中受益的基礎設施。_
...
- DataStax 收購了 Langflow，因為其無程式碼 AI 工作流建構器。
...
基於這樣的想法，我們最初確定了我們的目標受眾為資料工程師、資料科學家、AI 工程師和 AI 研究人員，這些需要建構非結構化資料 ETL 管道的 AI 建構者。

受到 dbt（主要是處理 ETL 中的 "T"）和 Airbyte（主要是 ELT 中的 "EL" 資料遷移）的啟發，我們最初建構了一個系統，將資料源和目的地與單個 DL 模型（無論是 LLM、STT/TTS 模型還是物件檢測模型）連接起來（[見圖 1](#figure-1)）。這是我們的第一個最小可行性產品（Minimum
...
例如，一個高精度的 PDF 文件分析器可能需要 PDF 解析器、VLM 或 OCR 模型來同時將圖表內容轉化為 Markdown 格式，然後使用 LLM 甚至擴散模型來生成最終的商業報告。
...
gRPC）、高度模組化的設計。後端核心使用 Go 實現，並提供 Python 和 TypeScript
SDK。選擇這樣的技術棧是為了模組化、可擴展性和可擴展性，同時也優先考慮了效能和安全性。

除了核心功能外，我們還建構了許多與 AI 和資料供應商的整合元件，使 Instill Core 內的非結構化資料能夠無縫進行重構和組合（[見圖 3](#figure-3)）。Instill
Core 主要由三個模組組成：

- Pipeline，用於非結構化資料 ETL。
- Model，用於託管 DL 模型。
- Artifact，用於有狀態服務資料儲存（例如，blob 儲存，向量資料庫）。

此時，Instill Core 提供了一個拖放式的無程式碼管道建構器，專為AI建構者量身定制（[見圖 4](#figure-4)）。
...
<figure id="figure-4">
  <img src="/blog/reimagine-unstructured-data-etl/no-code-pipeline-builder.png" alt="之前的 Instill Core 無程式碼管道建構器允許 AI 建構者無縫打造非結構化資料 ETL 管道，在畫布上可視化，並監控每個組件的輸入和輸出，具有完全的可觀察性。" />  
  <figcaption>圖 4. 之前的 Instill Core 無程式碼管道建構器允許 AI 建構者無縫打造非結構化資料 ETL 管道，在畫布上可視化，並監控每個組件的輸入和輸出，具有完全的可觀察性。</figcaption>
</figure>
...
再者，儘管無程式碼 UI/UX 能夠使不懂技術的使用者也能夠建構和運行資料管道，但我們的目標受眾其實是開發者，一群懂技術、會寫程式的人。作為開發者，我們知道一定有更好的方法。

### 低程式碼管道配方（2024年）

宣告式管理（Declarative
Management）為這樣的問題，提供了一個很好的解決方案。在進行了數千次拖放操作和無盡的滑鼠點擊建構管道後，我們決定是時候繼續前進，開始思考如何使用 YAML 配方來建構和維護管道。

會有這樣的想法，主要是受到我個人在管理 Kubernetes 時的經驗啟發。使用 Kubernetes 叢集總是令我感到安全且管理容易，因為 YAML 檔案可以版本控制並且具有高可讀性。透過 Terraform 等工具，我們還可以進一步實踐基礎設施即程式碼（Infrastructure
...
今天，你可以使用 Instill
Core 來建構聊天機器人、植物表型分析器、複雜 PDF 解析器、網站爬蟲，甚至為 AI 代理人工具建構進階 RAG 系統。等等。。。AI 代理人？這又是我在什麼賣弄學問的話術？其實不然，在 Instill
...
使用 YAML 配方建構管道確實非常棒，我們喜歡這個方案，我們的早期使用者也喜歡這種方式。但對於作為一個軟體產品開發愛好者，並具有高標準的我們，我們知道，一定有更好的方法！

### 對話式的 Instill AI（2025年）

現在就是未來，未來就是現在。

參考 GitHub Copilot、Cursor 和 Windsurf 為 AI 使用者直接生成程式碼，而 Bolt.new 和 Replit 甚至更進一步使他們能夠打造全端軟體。同樣地，Instill
...
- 非結構化資料 ETL：處理和提取大量非結構化資料（包括文件、網站、圖片、影片和聲音）中的有價值見解。
- 忠實性：獲得您可以信任的高保真答案。
...
作為一家小而美的新創公司，我們最大的優勢就是速度。但無論我們速度能有多快，如果方向是錯的，那我們將不會取得任何進展。我們本來可以追求僅針對 AI 建構者的無程式碼管道建構器，或是低程式碼管道配方這樣的產品，來進行市場佈局，但我們選擇不這樣做，相反的，我們重押了生成式 AI 的發展潛力與多功能性，平行同步與之一起演化我們的產品。這不僅僅是因為作為新創公司，輕巧的我們可以這麼做，也是因為我們清楚地知道我們想做什麼、該做什麼、能做什麼，以及市場真正的需求是什麼，我們完全有機會可以服務更廣大的 AI 使用者群。

這是一堂得來不易的產品旅程，我們一路走來，從為了 AI 建構者所打造的無程式碼到低程式碼解決方案，到現在將重點轉向為所有 AI 使用者提供基於對話式的 AI 代理人，希望讀者您能在這篇文章中找到一些有用的經驗借鏡。我一直有個想像，一種能夠理解任何物理訊號，並且有智慧地回應任何人類語言指令的電腦系統。如今這個願景比以往任何時候都更接近現實，我很慶幸能夠活在這一千載難逢機會的當下。
...
- 擴散模型（Diffusion Models），如 DALL-E 和 Stable Diffusion，通過逐步細化隨機雜訊以匹配給定的文字提示來生成圖片。

### 傳統資料 ETL vs. 非結構化資料 ETL

- 資料 ETL（Extract, Transform,
  Load，提取、轉換、載入）是一種資料整合過程，從各種來源提取資料，將其轉換以適應操作需求，並載入到目標系統（如資料倉庫）中。它也被稱為資料清洗或資料整理。
- 非結構化資料 ETL 涉及從各種非結構化來源（如文件、圖片或影片）提取資料，將其轉換為適合分析的結構化格式，並載入到資料倉庫或資料庫等目標系統中。兩者的逐項對照見[表 1](#table-1)。
...
| **提取來源**             | SQL 資料庫、CSV 文件、電子試算表。          | 文件、電子郵件、社交媒體、多媒體、日誌。                                                         |
...
| **載入目的地**           | 資料倉庫（Snowflake、BigQuery、Redshift）。 | 向量資料庫（Pinecone、ChromaDB）、文件儲存（Elasticsearch、MongoDB）、blob儲存（GCS、S3、MinIO） |
...
- AI 建構者是設計、開發和優化 AI 產品的開發者，利用機器學習、軟體開發和資料科學的技能來打造 AI 驅動的技術。相比之下，AI 使用者是利用這些 AI 產品來提高生產力的專業人士，將 AI 應用於自動化、內容打造、資料分析和決策等領域。建構者使用 PyTorch、TensorFlow 和 Hugging
  Face 等工具來建構 AI 解決方案，使用者則與 ChatGPT、GitHub
  Copilot 和 AI 驅動的分析工具等 AI 應用程式交互，以簡化他們的工作流程。兩種角色的對照見[表 2](#table-2)。

<figure id="table-2" class="table-figure">

| <div style="width:80px"> | **AI 建構者** 👩‍🔧👨🏻‍🔧                                                       | **AI 使用者** 👩‍💼👨🏻‍💼                                                              |
| ------------------------ | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------- |
| **定義**                 | 設計、開發和建構 AI 產品的人。                                           | 使用 AI 產品來提高生產力的人。                                                  |
| **主要目標**             | 打造和優化 AI 驅動的技術。                                               | 利用 AI 提高效率和決策能力。                                                    |
| **關鍵活動**             | 開發機器學習模型、微調 LLMs、整合 AI API、建構 AI 驅動的應用程式。       | 使用 AI 進行自動化、內容生成、資料分析、研究和溝通。                            |
| **所需技能**             | AI/ML 工程、軟體開發、資料科學、產品管理。                               | 各自領域的專業知識、有效與 AI 工具交互的能力。                                  |
| **示例**                 | OpenAI、Hugging Face、Google 或打造 AI 驅動應用程式的新創公司的工程師。  | 使用 ChatGPT 的作家，利用 AI 分析的行銷人員，使用 AI 輔助程式設計工具的編碼員。 |
